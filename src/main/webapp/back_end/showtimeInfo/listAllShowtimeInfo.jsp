...
<%@page import="com.service.ShowtimeInfoService"%>
...
<%  
    ...
    if(request.getAttribute("showtimeInfoListData")==null) pageContext.setAttribute("showtimeInfoListData",list);
%>
<!DOCTYPE html>
<html>
	<head>
	...
	<div class="main">
		...
		<form method="post" action="<%=request.getContextPath()%>/showtimeInfo/showtimeInfo.do">
			<a>播放日期:</a>
			<input type="date" name="starthiredate"><br>
			<a>影廳:</a>
			<select size="1" id="screenId">
			    <c:forEach var="Screen"  items="${showtimeInfoSvc.all}">
			    		<option value="${Screen.screen}">${Screen.screen.getScreenId()}</option>
				</c:forEach>    
			</select>
		 ...
		 <tbody>
			<c:forEach var="ShowtimeInfo" items="${showtimeInfoListData}" varStatus="s">
				<tr class="showtimeInfoStatusRow">
					...
					<td>
					  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/showtimeInfo/showtimeInfo.do" style="margin-bottom: 0px;">
