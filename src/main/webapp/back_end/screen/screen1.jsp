...
<meta charset="BIG5">
<title>A廳</title>
<link rel="stylesheet" href="./back_end/screen/screen1.css">
<link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css'
	rel='stylesheet'>
<link rel="stylesheet" href="./back_end/screen/management.css">
<link rel="stylesheet"
		...
		<ol class="cabin">
			...
			<li class="row row--A">
				<ol class="seats" type="A">
					...
					Pattern pattern = Pattern.compile("([A-N][0-9]+[N,Y,F])");
			...
			<li class="row row--<%=row%>">
				<ol class="seats" type=<%=row%>>
					...
					<li class="seat">
					<input type="checkbox"  id="<%=firstThreeLetter%>" name="seat" value="<%=eachSeat%>" disabled />
					...
					<li class="seat"><input type="checkbox" id="<%=firstThreeLetter%>" name="seat"
...
<script src="./back_end/screen/screen.js"></script>
 ...
 <script type="text/javascript">
        ...
        function connectWebSocket() {
            ws = new WebSocket("ws://localhost:8081/CIA10140-Webapp/seatSync");

            ws.onmessage = function(event) {
                var message = event.data;
                var seatInfo = message.split(',');
                var seatId = seatInfo[0];
                var seatStatus = seatInfo[1];
                document.getElementById(seatId).checked = (seatStatus === 'Y');
            };
        ...
        function toggleSeat(seatId, isChecked) {
            console.log("座位 " + seatId + " 的狀態改變為 " + (isChecked ? "已選" : "未選"));
