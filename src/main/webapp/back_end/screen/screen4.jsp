...
<meta charset="BIG5">
<title>D廳</title>
<link rel="stylesheet" href="./back_end/screen/screen4.css">
...
<jsp:include page="../management.jsp" />
	...
	<div class="theatre">
		<div class="screen-side">
			...
			<%		
			ShowtimeInfo show = (ShowtimeInfo)session.getAttribute("whichShow");	 //取得場次資訊
			...
			<li class="row row--A">
				<ol class="seats" type="A">
					...
					while (matcher.find()) {
						...
						String row = eachSeat.substring(0, 1); //取出ROW的第一個字母 A B C...etc
						String ableToBook = eachSeat.substring(3, 4); // N N N...etc
						String firstThreeLetter = eachSeat.substring(0, 3); //A01,A02....G08 etc
			...
			<li class="row row--<%=row%>">
				<ol class="seats" type=<%=row%>>
					...
					<li class="seat"><input type="checkbox"
						...
						value="<%=eachSeat%>" onclick="toggleSeat(this.id, this.checked);"/> 
						<label for="<%=firstThreeLetter%>"><%=firstThreeLetter%></label></li>
...
<script>
//選取 <li> 標籤
var liElement = document.querySelector('li.row--G');

// 創建新的 <pre> 元素
var preElement = document.createElement('pre');
preElement.className = 'aisleletter';
preElement.textContent = '                                  A                    i                s                l               e';
