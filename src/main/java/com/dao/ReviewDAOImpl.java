...
public class ReviewDAOImpl implements ReviewDAO {
	...
	public List<Review> getByMovie(Movie movie) {
		return getSession().createQuery("FROM Review WHERE movie = :movie ORDER BY reviewDate DESC", Review.class)
	...
	public List<Review> getByMem(Mem mem, int currentPage) {
		...
		return getSession().createQuery("FROM Review WHERE mem = :mem ORDER BY reviewDate DESC", Review.class)
	...
	public List<Review> getAll() {
		return getSession().createQuery("FROM Review ORDER BY reviewDate DESC", Review.class).list();
	}
	...
	public List<Review> getByCompositeQuery(Map<String, String> map, int currentPage) {
		...
		if (map.containsKey("startreviewDate") && map.containsKey("endreviewDate"))
			predicates.add(builder.between(root.get("reviewDate"), Timestamp.valueOf(map.get("startreviewDate")),
					Timestamp.valueOf(map.get("endreviewDate"))));
		...
		for (Map.Entry<String, String> row : map.entrySet()) {
			...
			if ("startreviewDate".equals(row.getKey())) {
				if (!map.containsKey("endreviewDate"))
					predicates.add(
							builder.greaterThanOrEqualTo(root.get("reviewDate"), Timestamp.valueOf(row.getValue())));
			}

			if ("endreviewDate".equals(row.getKey())) {
				if (!map.containsKey("startreviewDate"))
					predicates
							.add(builder.lessThanOrEqualTo(root.get("reviewDate"), Timestamp.valueOf(row.getValue())));
			}
	...
	public List<Review> getAll(int currentPage) {
		int first = (currentPage - 1) * PAGE_MAX_RESULT;
		return getSession().createQuery("FROM Review ORDER BY reviewDate DESC", Review.class)
				.setFirstResult(first)
				.setMaxResults(PAGE_MAX_RESULT)
				.list();
	}
